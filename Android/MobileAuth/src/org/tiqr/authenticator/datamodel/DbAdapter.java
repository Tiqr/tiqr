...
public class DbAdapter 
{
    ...
    private static final String JOIN_IDENTITY_IDENTITYPROVIDER = TABLE_IDENTITY + " JOIN " + TABLE_IDENTITYPROVIDER + 
    " ON " + TABLE_IDENTITY + "." + IDENTITYPROVIDER + " = " + TABLE_IDENTITYPROVIDER + "." + ROWID;
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper 
    {
        ...
        {
            db.execSQL("CREATE TABLE " + TABLE_IDENTITYPROVIDER + " (" + ROWID 
                    + " INTEGER PRIMARY KEY AUTOINCREMENT, "
                    + DISPLAY_NAME + " TEXT NOT NULL, "
                    + IDENTIFIER + " TEXT NOT NULL, "
                    + AUTHENTICATION_URL + " TEXT NOT NULL, "
                    + OCRA_SUITE + " TEXT NOT NULL, "
                    + INFO_URL + " TEXT NOT NULL, "
                    + LOGO + " BINARY);");
            
            db.execSQL("CREATE TABLE " + TABLE_IDENTITY + " (" + ROWID 
                    + " INTEGER PRIMARY KEY AUTOINCREMENT, "
     ...
     * 
     * The identity object's id is automatically set based on
    ...
    public Cursor getIdentityByIdentifierAndIdentityProviderId(String identifier, long identityProviderId) throws SQLException 
    {
        Cursor cursor =
            _db.query(
                TABLE_IDENTITY, 
                new String[] { ROWID, DISPLAY_NAME, BLOCKED, IDENTIFIER, IDENTITYPROVIDER, SORT_INDEX }, 
                IDENTIFIER + " = ? AND " + IDENTITYPROVIDER + " = ?", 
                new String[] { identifier, String.valueOf(identityProviderId) }, 
                null, 
                null, 
                null
            ); 
    ...
    public Identity getIdentityByIdentityId(long identity_id)
    {
        ...
        Cursor cursor =
            builder.query(_db, 
                          new String[] { TABLE_IDENTITY+"."+ROWID, TABLE_IDENTITY+"."+DISPLAY_NAME, TABLE_IDENTITY+"."+BLOCKED, TABLE_IDENTITY+"."+IDENTIFIER, TABLE_IDENTITY+"."+SORT_INDEX, TABLE_IDENTITYPROVIDER+"."+LOGO,  },
                          TABLE_IDENTITY+"."+ROWID+" = ?", 
                          new String[] { String.valueOf(identity_id) },
                          null,
                          null,
                          SORT_INDEX);
    ...
    /**
     * Returns a cursor for all identities available in the system. The
     * identities are ordered by their sort index.
     * 
     * @return cursor object
     */
    ...
    public Cursor findIdentitiesByIdentityProviderIdWithIdentityProviderData(long identityProviderId) throws SQLException
    {
        ...
        Cursor cursor =
            builder.query(
                _db, 
                new String[] { TABLE_IDENTITY+"."+ROWID, TABLE_IDENTITY+"."+DISPLAY_NAME, TABLE_IDENTITY+"."+BLOCKED, TABLE_IDENTITY+"."+IDENTIFIER, TABLE_IDENTITY+"."+SORT_INDEX, TABLE_IDENTITYPROVIDER+"."+LOGO,  },
                IDENTITYPROVIDER + " = ? AND " + TABLE_IDENTITY + "." + BLOCKED + " <> 1",
                new String[] { String.valueOf(identityProviderId) }, 
                null, 
                null, 
                SORT_INDEX
            ); 
    ...
    /**
     * Returns the identities for the given identity providers ordered by their sort index and returns them
     * as an array of Identity objects.
     *  
     * @return result array
     */
    ...
    /**
     * Returns the identities for the given identity provider ordered by their sort index and returns them
     * as an array of Identity objects.
     * 
     ...
     * TODO: logo
     * 
     * The identity provider object's id is automatically set based on
     * the new database row id.
     *  
    ...
    public Cursor getIdentityProviderByIdentifier(String identifier) throws SQLException 
    {
        Cursor cursor =
            _db.query(
                TABLE_IDENTITYPROVIDER, 
                new String[] { ROWID, DISPLAY_NAME, IDENTIFIER, AUTHENTICATION_URL, OCRA_SUITE, LOGO, INFO_URL }, 
                IDENTIFIER + " = ?", 
	...
	public IdentityProvider getIdentityProviderForIdentityId(long identity_id) {
		...
		Cursor cursor = builder.query(_db,
				new String[] {
					TABLE_IDENTITYPROVIDER + "." + ROWID,
