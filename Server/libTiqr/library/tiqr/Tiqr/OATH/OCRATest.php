...
class Tiqr_OATH_OCRATest extends PHPUnit_Framework_TestCase {

	/**
	 * @dataProvider RFCVectorsProvider
	 */
	public function testRFCVectors($ocrasuite, $key, $counter, $question, $password, $session, $timestamp, $expected_result) {
		$result = strval(OCRA::generateOCRA($ocrasuite, $key, $counter, $question, $password, $session, $timestamp));
	...
	public function RFCVectorsProvider() {
		...
		foreach($tests as $test) {
			...
			foreach($test['vectors'] as $vector) {
				...
				if (isset($vector['params']['C'])) {
					$counter = dechex($vector['params']['C']);
				}
				if (isset($vector['params']['Q'])) {
					$question = dechex($vector['params']['Q']);
				}
				if (isset($test['pin_sha1'])) {
					$password = $test['pin_sha1'];
				}
				if (isset($vector['params']['T'])) {
					$timestamp = dechex($vector['params']['T']);
				}

				$data[] = array($ocrasuite, $key, $counter, $question, $password, $session, $timestamp, $vector["result"]);
			}
		}
